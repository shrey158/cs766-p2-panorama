...
for i = 1:size(filelist,1)
    ...
    imgBuff(:,:,end) = 1;
    imgBuff(:,:,1:nClrs) = img;
    disp(['Removing distortion of ',filelist{i,1},' ...']);
    imgBuff = undistort(imgBuff,f,k1,k2,1);
    disp(['Reprojecting ',filelist{i,1},' to cylindrical coordinates ...']);
...
for i = 1:size(filelist,1)
    disp(['Extracting SIFT features of ',filelist{i,1},' ...']);
...
for i = 2:size(wkset,1)
    disp(['Estimating translation from ', ...
        filelist{i,1},' to ',filelist{i-1,1},' ...']);
    des1 = wkset(i-1).des;
...
disp('Removing vertical drift ...');
[~,nCols0,~] = size(wkset(1).imgBuff);
c1 = floor(nCols0/2);
imgBuff = imgBuff(:,c1:end-(nCols0-c1)+1,:);
drift = 0;
for i = 2:size(wkset,1)
    drift = drift + wkset(i).t2prev(1);
end
imgBuff = affine(imgBuff, drift);
imgBuff = imgBuff(:,1:end-1,:);

%% CROPPING THE IMAGE
% crop out black boundaries
disp('Cropping the panorama ...');
...
% crop out all rest no-scene area
r1 = 1;
...
disp('Finished and output to file.');